sap.ui.define([
	...
	return BaseController.extend("history.for.every.thing.ui.controller.App", {
			...
			var oActionSheet = new ActionSheet(this.getView().createId("userMessageActionSheet"), {
				...
				buttons: [
					new Button({
						...
						press: this.onLogoutButtonPress
					})
				],
				afterClose: function () {
		...
		onLogoutButtonPress: function (oEvent) {
			...
			var appController = sap.ui.getCore().byId("__xmlview0").getController();

			var oUserModel = appController.getModel("loggedUser");
			oUserModel.loadData("model/loggedUser.json");

			var sideMenuModel = appController.getModel("side");
			sideMenuModel.loadData("model/sideContent.json");

			var alertsModel = appController.getModel("alerts");
			alertsModel.loadData("model/alerts.json");					
			
			appController.getRouter().navTo("login");
		}
	});
});
